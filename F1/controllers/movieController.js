   ...
   Movie.find({mov_name:regex}, 'mov_name mov_thumb')
    ...
    .exec(function (err, list_movies) {
      if (err) {return next(err)} 
            ...
            Movie.find({mov_dir:regex}, 'mov_name mov_thumb')
    //.populate('mov_dir')
    .exec(function (err, list_movies) {
      if (err) {return next(err)} 
      else  if(list_movies==''){
            // Successful, so render
            //res.render('movie_list', { title: 'Not Found..!', movie_list:  list_movies});
            Movie.find({mov_actor:regex}, 'mov_name mov_thumb')
            //.populate('mov_dir')
            .exec(function (err, list_movies) {
              if (err) {return next(err)} 
              else  if(list_movies==''){
                  ...
                  res.render('movie_list', { title: 'Movies Found by Actor Input', movie_list:  list_movies});
                }
            });
            
        }
        else{
          res.render('movie_list', { title: 'Movies Found by Director Input', movie_list:  list_movies});
        }
    });
        }
        else{
